    ...
    <div className="mb-3 flex flex-wrap gap-2">
      {PROMPTS.map((p) => (
        <button
          ...
          type="button"
          onClick={() => onPick(p)}
          className="cursor-pointer rounded-full border border-zinc-300 px-3 py-1.5 text-xs text-zinc-700 hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-900"
        >
          {p}
        </button>
      ))}
